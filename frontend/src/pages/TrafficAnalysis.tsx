...
const TrafficAnalysis = () => {
  ...
  const fetchTraffic = useCallback(async (isRefresh = false) => {
    ...
    try {
      ...
      const trends = trendsRes.data as Array<{ time: string; congestion: number }>;
      const sectors = sectorsRes.data;
      const forecast = forecastRes.data.map((f: any) => ({ ...f, isForecast: true }));
      ...
      setTrafficData({
        ...
        active_alerts: sectors.filter((s: any) => s.alert).length,
