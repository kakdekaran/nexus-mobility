...
interface CongestionMetricsProps {
  data: any[];
}

const CongestionMetrics: React.FC<CongestionMetricsProps> = ({ data: apiData }) => {
  const chartData = apiData?.length > 0 ? apiData.map((row: any) => ({
    time: row.time,
    arterial: row.isForecast ? null : row.congestion,
    forecast: row.isForecast ? row.congestion : (row.time === apiData.find((d: any) => d.isForecast)?.time ? row.congestion : null),
